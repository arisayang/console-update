<template>
  <div id="app">
    ...
    <!-- Tiptap -->
    <div
      class="rounded-xl bg-grey px-6 py-2"
      :class="{
        'text--white': isDarkTheme,
        'file-input--darken': isDarkTheme,
        'lighten-3': !isDarkTheme,
      }"
    >
...
const parsedCell = (cell) => parsePhoneNumber(cell, { regionCode: 'TW' }).number?.international;
</script>
...
.v-application {
  ...
  * {
  ...
  .d-inline-flex {
    &.gap-sm {
      gap: map-get($vuetify, 'border-radius-root') / 2;
    }
    &.gap {
      gap: map-get($vuetify, 'border-radius-root');
    }
    &.gap-lg {
      gap: map-get($vuetify, 'border-radius-root') * 2;
    }
    &.gap-xl {
      gap: map-get($vuetify, 'border-radius-root') * 4;
    }
  }
  ...
  .swal2 {
    ...
    &-popup {
      ...
      &.swal2-show {
        animation: showSweetAlert 0.25s ease !important;
      }
      .swal2-content,
  ...
  .v-tooltip {
    &__content {
      padding: map-get($spacer, 4) map-get($spacer, 3);
      max-width: 300px;
      opacity: 1 !important;
      border-radius: map-get($spacer, 2);
      font-size: 12px;
      line-height: 1.5em;
      span {
